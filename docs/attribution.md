...
this project. Its use of explicit rollout lineage metadata before falling back
to counter heuristics was informed by CodexBar's scanner, but the JavaScript
parser and normalized output are original to this repository; no CodexBar or
ccusage parser file is vendored. The menu-bar pace projection and quota-alert
state behavior are small, schema-specific adaptations of ideas in the
MIT-licensed CodexBar sources identified below.
...
| Codex rollout lineage | [`CostUsageScanner.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCore/Vendored/CostUsage/CostUsageScanner.swift) | **Concept adapted in the local collector.** The estimator reads `session_meta` relationships, loads referenced ancestors, removes a child's matching replay prefix, and then applies counter deltas to novel turns. The JavaScript parser and ancestor-sequence matcher are original to this repository; CodexBar's Swift scanner, waterline implementation, cost model, and file-discovery code were not copied. |
| Daily token totals and breakdown | [`CostUsageModels.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCore/CostUsageModels.swift), [`CostUsageFetcher.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCore/CostUsageFetcher.swift), [`CostUsageScanner+CacheHelpers.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCore/Vendored/CostUsage/CostUsageScanner%2BCacheHelpers.swift), [`MenuCardView+Costs.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBar/MenuCardView%2BCosts.swift), and [`CLICostCommand.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCLI/CLICostCommand.swift) | **Information model adapted.** This dashboard now keeps “today” separate from quota pressure and preserves input, output, cached-input, reasoning-output, and model totals. Its normalized schema, web UI, and Swift rows were written for this project; no upstream daily-report or pricing implementation is vendored. |
...
The menu-bar adaptations above first shipped in version 0.8.0; the daily-token
separation and replay-prefix accounting shipped in version 0.8.1. The pinned
commit, copyright, and MIT permission text are preserved in

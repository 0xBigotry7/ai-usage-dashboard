...
| Codex rollout lineage | [`CostUsageScanner.swift`](https://github.com/steipete/CodexBar/blob/cc8da27cec92029a6435bfee4a703a719290234e/Sources/CodexBarCore/CostUsage/CostUsageScanner.swift) | **Concept adapted in the local collector.** The estimator trusts the current rollout's `session_meta` relationship fields to identify inherited counters, keeps rollout files as independent counter streams, and falls back conservatively for older metadata. CodexBar's Swift scanner, waterline implementation, cost model, and file-discovery code were not copied. |
